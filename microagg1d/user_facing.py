...
from microagg1d.cost_round import AdaptedRoundUpCostCalculator, RoundUpCostCalculator, RoundDownCostCalculator, AdaptedRoundDownCostCalculator
...
def _sse_staggered2(v, k, stable=1):
    ...
    n = len(v)
    if stable==2:
        cost_calculator = FasterAdaptedSSECostCalculator(calc_cumsum(v), k, np.empty(n+1, dtype=np.float64))
        return convert_implicit_to_explicit_clustering(method(n, cost_calculator, k))
    if stable==1:
        cost_calculator = StableAdaptedSSECostCalculator(v, k, np.empty(n+1, dtype=np.float64), 3*k)
        return convert_implicit_to_explicit_clustering(method(n, cost_calculator, k))
    elif stable==0:
        cumsum = calc_cumsum(v)
        cumsum2 = calc_cumsum(np.square(v))
        cost_calculator = AdaptedSSECostCalculator(cumsum, cumsum2, k, np.empty(n+1, dtype=np.float64))
        return convert_implicit_to_explicit_clustering(method(n, cost_calculator, k))
    else:
...
def _rounddown_user(v, k, algorithm):
    ...
    n = len(v)
    if algorithm=="galil_park":
        cost_calculator = AdaptedRoundDownCostCalculator(v, k, np.empty(n+1, dtype=np.float64))
        return convert_implicit_to_explicit_clustering(__galil_park2(n, cost_calculator))
    elif algorithm=="wilber":
        cost_calculator = AdaptedRoundDownCostCalculator(v, k, np.empty(n+1, dtype=np.float64))
        return convert_implicit_to_explicit_clustering(__wilber2(n, cost_calculator))
...
def _maxdist_user(v, k, algorithm):
    ...
    n = len(v)
    if algorithm=="galil_park":
        cost_calculator = AdaptedMaxDistCostCalculator(v, k, np.empty(n+1, dtype=np.float64))
        return convert_implicit_to_explicit_clustering(__galil_park2(n, cost_calculator))
    elif algorithm=="wilber":
        cost_calculator = AdaptedMaxDistCostCalculator(v, k, np.empty(n+1, dtype=np.float64))
        return convert_implicit_to_explicit_clustering(__wilber2(n, cost_calculator))
