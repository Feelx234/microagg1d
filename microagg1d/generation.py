...
def _create_sol(n,k,d):
    ...
    for i in range(d-1):
        delta = abs(l)//(d-i)
        solution[l:l+delta]=arange_len+i-d #arange_len-i-1
        l = l+delta
    solution[l:]=arange_len-1
    return solution
    ...
    if remainder==0:
        ...
        for d in range(2, d_max):
            ...
            for i in range(d-1):
                delta = abs(l)//(d-i)
                #print("LR", l, l+delta)
                solution[l:l+delta]=arange_len+i-d
                l = l+delta
            solution[l:]=arange_len-1
            costs[d] = compute_sse_sorted_stable(arr[-remainder+d*k:], solution[-remainder+d*k:])
...
def create_pair_known_sizes(sizes, k, epsilon=0):
    ...
    if ((max_size:=np.max(sizes))+1) * epsilon >= 1:
        raise ValueError(f"epsilon ({epsilon}) is to large because max(size)={max_size} and we need (max(size)+1)*epsilon < 1")
